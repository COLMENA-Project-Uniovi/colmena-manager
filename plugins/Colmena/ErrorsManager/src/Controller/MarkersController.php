...
class MarkersController extends AppController
{
  ...
  /**
   * Function which finds the markers that were produced during an especific session schedule
   *
   * @return
   */
  public function getMarkers()
  {
    # We obtain the scheduleID's value from the request
    $scheduleID = $this->request->getData('scheduleID');

    try {
      # We check if the schedule exists
      $schedule = $this->{$this->getName()}->Session->SessionSchedules->get($scheduleID);
    } catch (\Throwable $th) {
      # We return a NotFoundException with the message "The SessionSchedule doesn't exist"
      throw new NotFoundException("The SessionSchedule doesn't exist");
    }

    # Using the scheduleID, we obtain the markers that were produced in the session schedule
    $markers = $this->{$this->getName()}->Session->find('all')
      ->where(['id' => $schedule->session_id])
      ->contain([
        'Markers'
      ])
      ->first()
